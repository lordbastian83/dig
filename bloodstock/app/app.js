...
function renderScore(h, r) {
  ...
  el.innerHTML = `
    <h3>${esc(h.name)} — <span class="${r.verdict === 'BID' ? 'verdict-bid' : 'verdict-reject'}">${r.verdict}</span>
      <span class="score-chip" title="Quality score — drives the upside probability in the EV tree">Bio-Metric score ${Math.round(r.score * 100)}</span></h3>
...
$('#export-pdf').addEventListener('click', () => {
  ...
  w.document.write(`<!doctype html><html><head><meta charset="utf-8">
    <title>Bio-Metric Thoroughbreds — watchlist ${today}</title>
    <style>
      @page { margin: 16mm; }
    ...
    <h1>Bio-Metric Thoroughbreds <span class="r">watchlist</span></h1>
    ...
    <div class="foot">Generated by the Bio-Metric Thoroughbreds scanner. Hard max bids are limit orders — never chase past them.</div>
    </body></html>`);
  w.document.close();
...
function renderCatalogue() {
  ...
  tb.innerHTML = scored.map(({ h, r, nick }, i) => {
    ...
    return `<tr tabindex="0">
      ...
      <td class="ta-c">${heatPill(Math.round(r.score * 100), 'Bio-Metric score')}</td>
...
function renderCatDist(scored) {
  ...
  host.innerHTML = miChart('Official rating', ratings, 'mi-blue')
    ...
    + miChart('Bio-Metric score', vaults, 'mi-green')
...
function renderStats(rows) {
  ...
  strip.innerHTML =
    ...
    (top ? `<div class="stat stat-top"><span class="stat-lab">top ${radarSort === 'dubai' ? 'for Dubai' : 'value'}</span>` +
      ...
      `<span class="stat-topsub">score ${Math.round(top.r.score * 100)} · Dubai fit ${dubaiPct(top.r)}</span></div>` : '');
  strip.hidden = false;
}
...
function renderMarketIntel() {
  ...
  const lb = ranked.map(({ h, r }, i) => `<div class="mi-lb-row">
    ...
    <span class="mi-lb-vault" title="Bio-Metric score">${Math.round(r.score * 100)}</span>
  ...
  host.innerHTML = chart('Official rating', ratings, 'mi-blue')
    ...
    + `<div class="mi-chart mi-lb"><h4>Best-value shortlist</h4>
        <div class="mi-lb-head"><span></span><span>horse</span><span>fit</span><span>score</span><span>max bid</span></div>
        ${lb}</div>`;
  card.hidden = false;
}
...
function horseFigureHTML(h) {
  ...
  return `<div class="mp-analyse-panel">
    ...
    <div class="mp-figure-actions">
      ...
      <a class="mp-analyse" href="gallop.html${q}">🎥 Motion / gallop ▸</a>
      <a class="mp-analyse" href="pedigree.html${q}">🧬 Pedigree ▸</a>
      <a class="mp-analyse" href="mating.html${q}">⚭ Mating ▸</a>
    </div>
  </div>`;
}
  ...
  $('#modal-body').innerHTML = `
    <div class="mp-topline">
      ...
      <div class="mp-headtext">
        <div class="mp-head">
          <h3>${esc(h.name)}</h3>
          <span class="mp-score">score ${Math.round(r.score * 100)}</span>
...
function horseReport(h) {
  ...
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(h.name)} — Bio-Metric Thoroughbreds</title><style>
    ...
    <div class="band">
      ...
      <div class="tile"><b>${Math.round(r.score * 100)}</b><span>Bio-Metric score</span></div>
    ...
    <div class="foot">Bio-Metric Thoroughbreds · ${today} · hard max bids are limit orders — never chase past them · analysis, not financial advice · black type &amp; availability need a human check.</div>
    </body></html>`);
  w.document.close();
  setTimeout(() => w.print(), 400);
}
...
(function wireScanner() {
  ...
  if (exp) exp.addEventListener('click', () => {
    ...
    download(`biometric-scanner-${stamp}.csv`, 'text/csv', [head.join(','), ...lines].join('\n'));
  });
...
function openCompare(horsesIn) {
  ...
  const M = [
    ['Bio-Metric score', (h, r) => ({ n: Math.round(r.score * 100), t: Math.round(r.score * 100) }), true],
...
(function heroImage() {
  ...
  const img = $('#hero-img');
  // A saved photo shows over — and hides — the default Bio-Metric wireframe horse.
  if (saved && img && okImg(saved)) {
    img.src = saved; img.hidden = false;
    const wire = $('.hero-horse'); if (wire) wire.style.display = 'none';
  }
